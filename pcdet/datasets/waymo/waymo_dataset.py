...
class WaymoDataset(DatasetTemplate):
    ...
    def __getitem__(self, index):
        ...
        # added
        data_dict['context_name'] = info.get('context_name', str(-1))
        data_dict['timestamp_micros'] = info.get('timestamp_micros', int(-1))
    ...
    def generate_prediction_dicts(batch_dict, pred_dicts, class_names, output_path=None):
        ...
        def generate_single_sample_dict(box_dict):
            ...
            if 'first_stage_xy_per_scene' in box_dict.keys():                
                pred_dict['first_stage_xy_per_scene'] = box_dict['first_stage_xy_per_scene'].cpu().numpy()
            if 'gt_boxes_per_scene' in box_dict.keys():                
                pred_dict['gt_boxes_per_scene'] = box_dict['gt_boxes_per_scene'].cpu().numpy()
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        if kwargs['eval_metric'] == 'kitti':
            ...
            # kitti_ap_result_str, _ = kitti_eval(eval_det_annos, eval_gt_annos)
            # print('kitti_ap_result_str', kitti_ap_result_str)
        else:
