...
class StackSAModuleMSGTrans(nn.Module):

    def __init__(self, *, radii: List[float], nsamples: List[int], mlps: List[List[int]], pre_mlp: List[int],
                 use_xyz: bool = True, pool_method='max_pool'):
        """
        Args:
            radii: list of float, list of radii to group with
        ...
        self.q_mlp = nn.Sequential(
            nn.Linear(3, pre_mlp[1]),
            # nn.LayerNorm(pre_mlp[1]),
            nn.ReLU(),
            nn.Linear(pre_mlp[1], pre_mlp[1]),
            nn.Dropout(0.1)
        ).cuda()
        self.kv_mlp = nn.Sequential(
            nn.Linear(pre_mlp[0], pre_mlp[1]),
            # nn.LayerNorm(pre_mlp[1]),
            nn.ReLU(),
            nn.Linear(pre_mlp[1], pre_mlp[1]),
            nn.Dropout(0.1)
        ...
        self.attn = nn.MultiheadAttention(pre_mlp[1], 8).cuda()
    ...
    def forward(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, features=None, empty_voxel_set_zeros=True):
        ...
        for k in range(len(self.groupers)):
            ...
            _query = self.q_mlp(new_xyz).unsqueeze(0)
            _key_value = self.kv_mlp(new_features)


            new_features, _ = self.attn(_query, _key_value, _key_value)
            new_features = new_features.squeeze(dim=0)
            new_features_list.append(new_features)
