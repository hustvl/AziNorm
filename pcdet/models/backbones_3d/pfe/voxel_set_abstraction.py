...
class VoxelSetAbstraction(nn.Module):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
        ...
        self.using_transformer_getting_point_features_from_multi_sources = \
            model_cfg.get('using_transformer_getting_point_features_from_multi_sources', False)
        ...
        for src_name in self.model_cfg.FEATURES_SOURCE:
            ...
            if self.using_transformer_getting_point_features_from_multi_sources:
                cur_layer = pointnet2_stack_modules.StackSAModuleMSGTrans(
                    radii=SA_cfg[src_name].POOL_RADIUS,
                    nsamples=SA_cfg[src_name].NSAMPLE,
                    mlps=mlps,
                    pre_mlp=SA_cfg[src_name].pre_mlp,
                    use_xyz=True,
        ...
        if 'raw_points' in self.model_cfg.FEATURES_SOURCE:
            ...
            if self.using_transformer_getting_point_features_from_multi_sources:
                self.SA_rawpoints = pointnet2_stack_modules.StackSAModuleMSGTrans(
                    radii=SA_cfg['raw_points'].POOL_RADIUS,
                    nsamples=SA_cfg['raw_points'].NSAMPLE,
                    mlps=mlps,
                    pre_mlp=SA_cfg['raw_points'].pre_mlp,
                    use_xyz=True,
                    pool_method='max_pool'
                )
